...
int log_skip_cb(void *ambit_object, ambit_log_header_t *log_header)
{
    ...
    printf("Got log header \"%s\" %d-%02d-%02d %02d:%02d:%02d\n", log_header->activity_name, 
        log_header->date_time.year, log_header->date_time.month, log_header->date_time.day, 
        log_header->date_time.hour, log_header->date_time.minute, log_header->date_time.msec/1000);
    if (log_count++ > 1) {
        return 0;
    }
    return 1;
...
void log_data_cb(void *object, ambit_log_entry_t *log_entry)
{
    printf("Got log entry \"%s\" %d-%02d-%02d %02d:%02d:%02d\n", log_entry->header.activity_name, 
        log_entry->header.date_time.year, log_entry->header.date_time.month, log_entry->header.date_time.day, 
        log_entry->header.date_time.hour, log_entry->header.date_time.minute, log_entry->header.date_time.msec/1000);
    int i;
    for (i=0; i<log_entry->header.samples_count; i++) {
        printf("Sample #%d, type: %d, time: %04u-%02u-%02u %02u:%02u:%2.3f\n", i, log_entry->samples[i].type, 
            log_entry->samples[i].utc_time.year, log_entry->samples[i].utc_time.month, log_entry->samples[i].utc_time.day, 
            log_entry->samples[i].utc_time.hour, log_entry->samples[i].utc_time.minute, (1.0*log_entry->samples[i].utc_time.msec)/1000);
    }
}

void mydump(void *object,  ambit_log_skip_cb skip_cb, ambit_log_push_cb push_cb ){

    libambit_log_read(object, skip_cb, push_cb, NULL, object);
